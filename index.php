...
<head>
	...
	<style type="text/css">
		* {
		...
		#canvas #icon #iconImage {
			margin: 0 auto;
			z-index: 10;
			background: #000;
			-webkit-mask: url('mmtlp.svg') center 0 / contain repeat;
			width: 400px;
			height: 400px
		}

		#canvas #icon .icon-text {
			text-align: center;
			height: 100%;
			width: 100%;
...
<body>
	...
	<div id="canvas">
		...
		<div class="right" id="inputs">
				...
				<table>
					<tr>
						...
						<td>
							...
							<input type="number" value="180" min="0" max="360" id="gradientRadius"
			...
			<div id="gradient-icon-options" style="display:none">
				<table>
					<tr>
						...
						<td>
							<label>Color</label>
							<input type="color" value="#c31313" id="gradientIconColor"
								onchange="ToggleIconGradient();" />
						</td>
						<td>
							<label>Degrees (Linear)</label>
							<input type="number" value="180" min="0" max="360" id="gradientIconRadius"
								onchange="ToggleIconGradient();" />
						</td>
					</tr>
				</table>
			...
			<div id="shadow-options" style="display:none">
				<table>
					<tr>
						...
						<td>
							<label>Spread</label>
							<input type="number" value="100" min="1" max="200" id="txtShadowSpread"
			...
			<table>
				<tr>
					<td>
						<label>Color</label>
						<input type="color" value="#FFFFFF" id="txtColor" value="#FFFFFF"
							onchange="ChangeTextColor();" />
					</td>
					<td>
						<label>Dropshadow</label>
						<input type="color" value="#000000" id="txtDropshadowColor" value="#FFFFFF"
							onchange="ChangeDropshadowColorAndSize();" />
					</td>
					<td>
						<label>Spread</label>
						<input type="number" value="4" min="1" max="25" id="txtDropshadowSpread" value="#FFFFFF"
							onchange="ChangeDropshadowColorAndSize();" />
					</td>
				</tr>
			</table>
			...
			<table>
				<tr>
					...
					<td>
						<label>Spacing</label>
						<input type="number" value="0" min="0" max="100" id="txtSpacing"
							onchange="ChangeTextSpacing();" />
					</td>
					<td>
						<label>Height</label>
						<input type="number" value="52" min="0" max="100" id="txtHeight"
							onchange="ChangeTextLineheight();" />
					</td>
					<td>
						<label>Vertical Tuning</label>
						<input type="number" value="42" min="0" max="100" id="txtPosition"
							onchange="ChangeTextVertical();" />
					</td>
				</tr>
			</table>
			<hr />
			...
			<div class="image-overlays">
				...
				<label>Enter background Image URL:</label>
				<input id="addBg" type="text" value="" placeholder="Enter external URL" /><br />
				<button onclick="AddBg();">Add Image</button><br />
				<div id="layers"></div>
		...
		<br />
		<center>
			Use your device screenshot feature to capture the image.<br />No back-end code or front-end analytics are
			used for this tool.
			<br /><br />
	...
	<script>
		...
		function AddBg() {
			if ($('#addBg').val() != '') {
				var i = $('#addBg').val();
				$('#icon').append('<img class="overlay" src="' + i + '" id="overlay-' + bgZIndex + '" style="z-index:' + bgZIndex + '" />');
				$('#overlay-' + bgZIndex).resizable();
				$('#overlay-' + bgZIndex).draggable();
				var newImg = '<table class="layer" id="layer-' + bgZIndex + '"><tr><td><img src="' + i + '" /></td><td><label>Scaling</label><input class="scaling" type="number" min="0" max="200" value="1" onchange="ChangeLayerScaling(' + bgZIndex + ');" /><td><label>Transparency</label><input type="number" min="0" value="1.00" max="1" step="0.01" onchange="ChangeLayerTransparency(' + bgZIndex + ');" class="transparency" /></td></tr></table>';
				$('#layers').append(newImg);
				$('#addImage').val('');
				bgZIndex += 1;
			}
